...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (laufband) {
    ...
    /* Die Bausteine werden einzeln erzeugt und die Maske ueber das
       Stilobjekt gesetzt — nicht als style-Attribut im Text. Sonst
       blockiert die Inhaltsregel (CSP) in der .htaccess die Logos. */
    PARTNER.concat(PARTNER).forEach(function (p, i) {
      ...
      var pfad  = BASIS + 'assets/partners/' + p.datei;
      var el;
      ...
      if (p.url) {
        ...
        el.target = '_blank';
        el.rel = 'noopener';
        if (kopie) { el.tabIndex = -1; el.setAttribute('aria-hidden', 'true'); }
        else { el.setAttribute('aria-label', p.name + ' \u2014 Website \u00f6ffnen'); }
      } else {
        el = document.createElement('div');
        el.setAttribute('role', 'img');
        if (kopie) el.setAttribute('aria-hidden', 'true');
        else el.setAttribute('aria-label', p.name);
      }

      el.className = 'partner-logo';
      el.title = p.name;
      el.setAttribute('data-datei', p.datei);
      el.style.maskImage = 'url(' + pfad + ')';
      el.style.webkitMaskImage = 'url(' + pfad + ')';
      laufband.appendChild(el);
    });
  ...
  /* ---------- 6. Kontaktformular ------------------------------------------
     ...
     SCHARF SCHALTEN (sobald kontakt.php auf dem Webspace liegt):
       1. FORMULAR_SCHAUMODUS auf false setzen
       2. im HTML beim <form> ergaenzen: action="kontakt.php" method="post"
     Alles andere laeuft dann von selbst. */
  (function () {
    ...
    /* Zwei technische Felder, die der Server zur Pruefung braucht:
         zeitstempel   wann das Formular geoeffnet wurde (Zeitsperre)
         captcha_pruef signierte Loesung der Rechenaufgabe, damit der Server
                       nachrechnen kann, ohne eine Sitzung zu eroeffnen */
    function verstecktesFeld(name) {
    ...
    var feldZeit  = verstecktesFeld('zeitstempel');
    var feldPruef = verstecktesFeld('captcha_pruef');
    feldZeit.value = String(Date.now());

    /* SHA-256 von "kairos" + Ergebnis — dieselbe Rechnung macht kontakt.php */
    function signiere(wert) {
      if (!window.crypto || !crypto.subtle) return Promise.resolve('');
      var daten = new TextEncoder().encode('kairos' + wert);
    ...
    function neueAufgabe() {
      ...
      hinweis.hidden = true;
      signiere(ergebnis).then(function (h) { feldPruef.value = h; });
    }

    neueAufgabe();
    neu.addEventListener('click', neueAufgabe);

    /* Kommt der Besucher mit ?fehler=… zurueck, sagen wir woran es lag */
    (function () {
      var code = new URLSearchParams(location.search).get('fehler');
      if (!code) return;
      var texte = englisch ? {
        '1': 'Something was missing or too long. Please check your entries.',
        '2': 'The message could not be sent. Please write to office@kairos-origin.com.',
        '3': 'Too many enquiries from this connection. Please try again in an hour.',
        '4': 'The security question was not answered correctly.'
      } : {
        '1': 'Es fehlte etwas oder ein Feld war zu lang. Bitte prüfen Sie Ihre Angaben.',
        '2': 'Die Nachricht konnte nicht versendet werden. Bitte schreiben Sie an office@kairos-origin.com.',
        '3': 'Zu viele Anfragen von diesem Anschluss. Bitte in einer Stunde erneut versuchen.',
        '4': 'Die Sicherheitsfrage wurde nicht richtig beantwortet.'
      };
      meldung.hidden = false;
      meldung.classList.add('fehler');
      meldung.textContent = texte[code] || texte['1'];
    }());
